...
export default function CooperativeSubmissionForm() {
  ...
  const handleListingChange = (e) => setListing({ ...listing, [e.target.name]: e.target.value });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!listing.commodityId || !listing.quantity || !listing.grade || !listing.pickupLocation) {
    ...
    setSelectedMembers([]);
    setListing({ commodityId: '', quantity: '', unit: 'Quintal', grade: '', pickupLocation: '' });
  };

  return (
    <div className="max-w-3xl mx-auto px-6 py-8 w-full">
      <h1 className="text-2xl font-bold text-stone-800 mb-1">Cooperative Submission</h1>
      <p className="text-stone-500 text-sm mb-6">Submit produce on behalf of cooperative members.</p>

      <form onSubmit={handleSubmit} className="bg-white border border-stone-200 rounded-lg shadow-sm p-6 flex flex-col gap-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Submitted By (Rep)</label>
            <input
              value={repName}
        ...
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-2">Members Included in This Submission</label>
          <div className="border border-stone-200 rounded-md divide-y divide-stone-100">
            {members.map((m) => (
              <label key={m.id} className="flex items-center gap-3 px-3 py-2.5 text-sm cursor-pointer hover:bg-stone-50">
                <input
                  type="checkbox"
        ...
        <div className="border-t border-stone-100 pt-5">
          <p className="text-sm font-medium text-stone-700 mb-3">Listing details apply to each selected member.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Crop</label>
              <select
                name="commodityId"
        ...
        <button
          type="submit"
          className="bg-green-800 text-white font-semibold rounded-md px-4 py-2.5 text-sm hover:bg-green-900 transition"
        >
          Submit for Members
        </button>

        {status?.type === 'error' && (
          <p className="bg-red-50 text-red-700 rounded-md px-3 py-2 text-sm">{status.message}</p>
        )}
        {status?.type === 'success' && (
          <p className="bg-green-50 text-green-800 rounded-md px-3 py-2 text-sm">
            ✅ Submitted listings for {status.count} member{status.count > 1 ? 's' : ''}.
          </p>
        )}
      </form>
    </div>
