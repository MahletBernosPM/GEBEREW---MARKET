 ...
 const [form, setForm] = useState({
  ...
  useEffect(() => {
    const runSync = async () => {
     ...
     setStatus((prev) => (prev?.type === 'queued' ? { type: 'success', crop: prev.crop } : prev));
    };
    runSync();
...
return (
  ...
  />
    ...
    <div>
      <label className="block text-sm font-medium text-stone-700 mb-1">Product Photo</label>
      ...
      {photo && (
        <div className="mt-2">
          <img src={photo} alt="Preview" className="w-20 h-20 object-cover rounded-lg border border-stone-200" />
          <button
            type="button"
